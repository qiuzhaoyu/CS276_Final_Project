...
medseg_train_images, medseg_train_masks, medseg_val_images, medseg_val_masks = split_data(
    images_medseg, masks_medseg_recover, split_ratio=0.9
)

radiopedia_train_images, radiopedia_train_masks, radiopedia_val_images, radiopedia_val_masks = split_data(
    images_radiopedia, masks_radiopedia_recover, split_ratio=0.9
)
...
scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
    optimizer,
    mode='min',           # 监控的指标是越低越好，例如验证损失
    factor=0.8,           # 每次减少学习率的因子
    patience=2,           # 如果验证指标在连续 5 个 epoch 不下降，则减少学习率
    threshold=0,       # 改善幅度小于该值时认为没有改善
    min_lr=1e-5,          # 最小学习率
...
def fit(epochs, model, train_loader, val_loader, criterion, optimizer, scheduler, device):
    ...
    for epoch in range(epochs):
        ...
        # ---------- 打印日志 ----------
        print(f"\nEpoch [{epoch+1}/{epochs}] | current_lr: {optimizer.param_groups[0]['lr']:.8f}")
